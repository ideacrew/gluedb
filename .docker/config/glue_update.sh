...
set +e
batch_handler=$( kubectl get pods | grep edidb-sidealong-glue-batch | grep Running )
set -e
if [ -z "$batch_handler" ]; then
  kubectl patch cronjobs edidb-sidealong-glue-batch -p "{\"spec\" : {\"suspend\" : true }}"
  curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "'\`' ### GlueDB Sidealong Update Started ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
else
  exit 5
fi
## bring down the listeners
echo "bringing down listeners: "$(date)
kubectl scale --replicas=0  deployment/edidb-sidealong-enrollment-validator deployment/edidb-sidealong-broker-updated-listener \
                            deployment/edidb-sidealong-policy-id-list-listener deployment/edidb-sidealong-enrollment-event-listener \
                            deployment/edidb-sidealong-enrollment-event-handler
sleep 60
kubectl scale --replicas=0 deployment/edidb-sidealong-enroll-query-result-handler
sleep 120
kubectl scale --replicas=0 deployment/edidb-sidealong-employer-workers
sleep 120
kubectl scale --replicas=0 deployment/edidb-sidealong-legacy-listeners
sleep 180
...
if [ "$update_status" -eq 1 ]; then
  curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "'\`' ### GlueDB Sidealong Update Completed :: Running Reports Before Starting Listeners ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
  ...
  kubectl scale --replicas=1 deployment/edidb-sidealong-legacy-listeners
  messages=1
  while [ $messages -gt 0 ]
  do
    sleep 120 
  ...
  kubectl scale --replicas=1 deployment/edidb-sidealong-employer-workers
  sleep 120
  kubectl scale --replicas=1 deployment/edidb-sidealong-enroll-query-result-handler
  sleep 120
  kubectl scale --replicas=1 deployment/edidb-sidealong-enrollment-validator deployment/edidb-sidealong-broker-updated-listener \
                             deployment/edidb-sidealong-policy-id-list-listener deployment/edidb-sidealong-enrollment-event-listener \
                             deployment/edidb-sidealong-enrollment-event-handler
  sleep 120
  kubectl patch cronjobs edidb-sidealong-glue-batch -p "{\"spec\" : {\"suspend\" : false }}"
  kubectl rollout restart deployment edidb-sidealong-$ENV_NAME
else
  exit 1
fi
EOL
...
then
  if [ "$curlTest" == "200" ]
  then 
    curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "'\`' ### GlueDB Sidealong Update Completed :: Listeners Are Up ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
    send_sms_notification Success
    exit 0
  else
    curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "'\`' ### GlueDB Sidealong Update Completed :: But Restart Failed ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
    send_sms_notification "Restart Failed"
    exit 1
  fi
elif [ "$update_status" -eq 5 ]
then 
  curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "<!channel> '\`' ### GlueDB Sidealong Update Did Not Start -- Batch Handler Is Running ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
  send_sms_notification "Did Not Start" "The batch handler is running!"
  exit 1
else
  curl -X POST --data-urlencode 'payload={"channel": "#'$SLACK_CHANNEL'", "username": "EDI Database Bot", "text": "<!channel> '\`' ### GlueDB Sidealong Update Failed ### '\`'", "icon_emoji": ":gear:"}' https://hooks.slack.com/services/$SLACK_TOKEN
