...
module Generators::Reports
  ...
  # To generate irs yearly policies need to send a run time calendar_year params i.e. Generators::Reports::IrsYearlySerializer.new({calendar_year: 2021, generate_pdf: true}) instead sending hard coded year
    ...
    def valid_policy?(policy, h41 = nil)
      return true if h41.present? || @notice_params[:type] == 'void'
    ...
    def process_policy(policy, h41 = nil)
       if valid_policy?(policy, h41)
        ...
        create_report_names unless h41
    ...
    def create_individual_h41_folder
      ...
      @irs_xml_path = ''
      @h41_folder_name ||= "#{irs_h41_source_sbm_id}.DSH.EOYIN.D#{Date.today.strftime('%Y%m%d')[2..-1]}.T#{Time.now.strftime("%H%M%S") + "000"}.P.IN"
