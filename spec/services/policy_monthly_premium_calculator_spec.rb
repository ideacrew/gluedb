...
describe Services::PolicyMonthlyPremiumCalculator do
  ...
  subject(:prem_amount_calculator) { Services::PolicyMonthlyPremiumCalculator.new(policy_disposition: policy_disposition, calendar_year: calendar_year)}
  ...
  context 'A child added on a policy in middle of the month' do
    ...
    it 'returns the policy prorated amount in the 3rd month of the calendar_year' do
      ...
      policy.enrollees.where(rel_code: "child").first.update_attributes!(pre_amt: "200", coverage_start: Date.new(calendar_year, 3, 16) )
  ...
  context 'A child dropped from the policy in middle of the month' do
    ...
    it 'returns the policy prorated amount in the 3rd month of the calendar_year' do
      policy.update_attributes!(pre_amt_tot: "300", tot_res_amt: "300", tot_emp_res_amt: "0", allocated_aptc: "0", elected_aptc: "0", applied_aptc: "0")
      policy.enrollees.where(rel_code: "self").first.update_attributes!(pre_amt: "300")
      policy.enrollees.where(rel_code: "child").first.update_attributes!(pre_amt: "200", coverage_end: Date.new(calendar_year, 3, 15))
      expect(policy.enrollees.count).to eq 2
