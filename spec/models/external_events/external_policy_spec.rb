...
context "Given a reinstated policy cv, on reinstate carrier term flag needs to reset", :dbclean => :after_each do
  ...
  it "creates new policy on # persist and creates APTC Credits table with APTC" do
    previous_policy = Policy.where(:hbx_enrollment_ids  => {"$in" => ['123']}).first
    expect(Policy.where(:hbx_enrollment_ids => eg_id).count).to eq 0

    expect(previous_policy.term_for_np).to eq true
    expect(previous_policy.subscriber.termed_by_carrier).to eq true
    ...
    policy = Policy.where(:hbx_enrollment_ids => eg_id)
    expect(policy.count).to eq 1
    expect(policy.first.aasm_state).to eq "resubmitted"

    previous_policy.reload
    expect(previous_policy.term_for_np).to eq false
    expect(previous_policy.subscriber.termed_by_carrier).to eq false
  end
end
