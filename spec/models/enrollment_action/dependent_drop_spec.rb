...
describe EnrollmentAction::DependentDrop, "given a qualified enrollment set, being published" do
  ...
  let(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, coverage_end: :one_month_ago) }
  let(:enrollee_new) { double(:m_id => 2, :coverage_start => :one_month_ago, :coverage_end => :one_month_ago) }
  let(:carrier) { instance_double(Carrier, :renewal_dependent_drop_transmitted_as_renewal => true) }
  let(:plan) { instance_double(Plan, :id => 1, ) }
...
describe "given prospective dependent drop", :dbclean => :after_each do
  ...
  let(:active_enrollee1) { Enrollee.new(m_id: primary.authority_member.hbx_member_id, rel_code: 'self', coverage_start: Date.today.beginning_of_year, coverage_end: '')}
  let(:active_enrollee2) { Enrollee.new(m_id: dep.authority_member.hbx_member_id, rel_code: 'child', coverage_start: Date.today.beginning_of_year, coverage_end: '')}
         ...
         <affected_members>
           <affected_member>
             ...
             <benefit>
               <premium_amount>465.13</premium_amount>
               <begin_date>#{dep_coverage_drop_date.strftime("%Y%m%d")}</begin_date>
               <end_date>#{dep_coverage_drop_date.strftime("%Y%m%d")}</end_date>
             </benefit>
           </affected_member>
           <affected_member>
             <member>
  ...
  subject do
    EnrollmentAction::DependentDrop.new(termination_event, event_after_dependent_drop)
  end
