...
describe EnrollmentAction::RetroDependentAddToActive, "given an enrollment event set that:
...
- events are adjacent", :dbclean => :after_each do
  ...
  subject { EnrollmentAction::RetroDependentAddToActive.new(retro_term_event, retro_dep_add_event, prospective_cancel_event) }
  ...
  context "#qualifies #persist #publish", :dbclean => :after_each do
    ...
    it "should persist and adds dependent to policy for retro action" do
      expect(EnrollmentAction::RetroDependentAddToActive.qualifies?([retro_term_event, prospective_cancel_event, retro_dep_add_event])).to be_truthy

      expect(active_policy.enrollees.count).to eq 1 #before persist
      expect(subject.persist).to be_truthy
      active_policy.reload
      expect(active_policy.policy_end).to eq nil
      expect(active_policy.enrollees.count).to eq 2 #after persist
