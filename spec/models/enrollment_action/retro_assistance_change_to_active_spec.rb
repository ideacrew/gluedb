...
describe EnrollmentAction::RetroAssistanceChangeToActive, "given an enrollment event set that:
 - three events
 - is the same plan
 ...
 - events occured with in the year", :dbclean => :after_each do
  ...
  let!(:active_policy) {
    ...
    policy.update_attributes(enrollees: [active_enrollee], hbx_enrollment_ids: ["123", "456"], applied_aptc: "100.00")
    policy.save
    policy
  }
  ...
  subject { EnrollmentAction::RetroAssistanceChangeToActive.new(retro_term_event, retro_aptc_change_event, prospective_cancel_event) }
  ...
  context "#qualifies #persist #publish", :dbclean => :after_each do
    ...
    it "should persist and update aptc on policy for retro action" do
      expect(EnrollmentAction::RetroAssistanceChangeToActive.qualifies?([retro_term_event, prospective_cancel_event, retro_aptc_change_event])).to be_truthy

      expect(active_policy.applied_aptc.to_s).to eq "100.0" #before persist
      expect(subject.persist).to be_truthy
      active_policy.reload
      expect(active_policy.policy_end).to eq nil
      expect(active_policy.applied_aptc.to_s).to eq "50.0" #before persist

      expect_any_instance_of(EnrollmentAction::ActionPublishHelper).to receive(:set_event_action).with("urn:openhbx:terms:v1:enrollment#change_financial_assistance")
      subject.publish
    end
  end
