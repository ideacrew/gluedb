...
describe EnrollmentAction::RetroContinuityAndTerm, "given an EnrollmentAction array that:
  - has one element that has retro new coverage
  - has one element that has terminated current coverage
  - has one element that has renewal for prior coverage" do
  let(:eg_id) { '1' }
  let(:carrier_id) { '1' }
  ...
  context "qualifies", :dbclean => :after_each do
    ...
    it "should return true" do
      expect(subject.qualifies?([action, termination, additional_action])).to be_truthy
    end
  end
  ...
  context "persist", :dbclean => :after_each do
    ...
    subject do
      EnrollmentAction::RetroContinuityAndTerm.new(termination, action, additional_action)
    end
    ...
    it "should create renewal policy" do
      subject.persist
      policy = Policy.where(hbx_enrollment_ids: additional_action.hbx_enrollment_id).first
      expect(policy.hbx_enrollment_ids).to eq(["678"])
      expect(policy.policy_end).to eq(nil)
      expect(policy.future_active?).to eq(true)
    end
  end
  ...
  context "publish", :dbclean => :after_each do
    ...
    let!(:renewal_action_helper) do
      EnrollmentAction::ActionPublishHelper.new(additional_action.event_xml)
    end
    ...
    before :each do
      ...
      allow(EnrollmentAction::ActionPublishHelper).to receive(:new).with(additional_action.event_xml).and_return(renewal_action_helper)
      ...
      allow(renewal_action_helper).to receive(:keep_member_ends).with([])
      allow(renewal_action_helper).to receive(:set_event_action).with("urn:openhbx:terms:v1:enrollment#auto_renew")
      allow(subject).to receive(:publish_edi).with(amqp_connection, renewal_action_helper.to_xml, additional_action.hbx_enrollment_id, additional_action.employer_hbx_id).and_return([true, {}])
    end
    ...
    it "sets event for auto renew action helper" do
      expect(renewal_action_helper).to receive(:set_event_action).with("urn:openhbx:terms:v1:enrollment#auto_renew")
      subject.publish
    end
    ...
    it "sets keep member ends for renewal action helper" do
      expect(renewal_action_helper).to receive(:keep_member_ends).with([])
      subject.publish
    end

    it "publishes termination & initial & renewal resulting xml to edi" do
      expect(subject).to receive(:publish_edi).exactly(3).times
      subject.publish
    end
  end
end