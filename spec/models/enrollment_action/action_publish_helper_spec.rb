...
describe EnrollmentAction::ActionPublishHelper, "told to set affected and enrolles end dates, when member node exists" do
  ...
  let(:carrier_member_id) { enrollee_primary.c_id }
  let(:action_publish_helper) { ::EnrollmentAction::ActionPublishHelper.new(source_event_xml) }
  let(:alias_ids) { [carrier_member_id, carrier_policy_id] }
  let(:carrier_member_id) { "urn:openhbx:hbx:me0:resources:v1:person:member_id##{enrollee_primary.c_id}" }
  let(:carrier_policy_id) { "urn:openhbx:hbx:me0:resources:v1:person:policy_id##{enrollee_primary.cp_id}" }

  context 'alias_ids nodes does exist' do
    let(:source_event_xml) { <<-EVENTXML
    <enrollment_event_body xmlns="http://openhbx.org/api/terms/1.0">
      <affected_members>
        <affected_member>
          <member>
            <id>
              <id>urn:openhbx:hbx:dc0:resources:v1:person:hbx_id#1</id>
              <alias_ids>
                <alias_id>
                  <id>urn:openhbx:hbx:me0:resources:v1:person:member_id#100203510</id>
                </alias_id>
                <alias_id>
                  <id>urn:openhbx:hbx:me0:resources:v1:person:policy_id#100207010</id>
                </alias_id>
              </alias_ids>
            </id>
          </member>
        ...
        <affected_member>
          <member>
            <id><id>urn:openhbx:hbx:dc0:resources:v1:person:hbx_id#2</id></id>
          </member>
          <benefit>
            <premium_amount>465.13</premium_amount>
            <begin_date>2019101</begin_date>
    ...
    <enrollees>
      <enrollee>
        <member>
          <id><id>urn:openhbx:hbx:dc0:resources:v1:person:hbx_id#1</id>
              <alias_ids>
                <alias_id>
                  <id>urn:openhbx:hbx:me0:resources:v1:person:member_id#100203510</id>
                </alias_id>
    ...
    context 'set carrier assigned ids at member level' do
      context 'enrollee has new carrier_member_id and carrier_policy_id' do
        let(:transformed_target_xml) {
          action_publish_helper.set_member_level_carrier_assigned_ids(enrollee_primary)
          Nokogiri::XML(action_publish_helper.to_xml)
        }
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, :c_id => '123456', :cp_id => '7123456') }

        it 'should replace alias_ids for enrollee' do
          transformed_target_xml.xpath("//cv:enrollment_event_body/cv:affected_members/cv:affected_member", xml_namespace).each do |node|
            node.xpath("cv:member/cv:id/cv:alias_ids/cv:alias_id/cv:id", xml_namespace).each do |id_node|
              expect(alias_ids).to include(id_node.content)
            end
          end
        end
      end
    ...
    context 'set carrier assigned ids at policy level' do
      context 'enrollee has new carrier_member_id and carrier_policy_id' do
        let(:transformed_target_xml) {
          action_publish_helper.set_policy_level_carrier_assigned_ids(enrollee_primary)
          Nokogiri::XML(action_publish_helper.to_xml)
        }
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, :c_id => '123456', :cp_id => '7123456') }
  ...
  context 'alias_ids nodes does not exist' do
    let(:source_event_xml) { <<-EVENTXML
    <enrollment_event_body xmlns="http://openhbx.org/api/terms/1.0">
      <affected_members>
        <affected_member>
    ...
    context 'set carrier assigned ids at member level' do
      ...
      context 'enrollee has both carrier_member_id and carrier_policy_id' do
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, :c_id => '123456', :cp_id => '7123456') }

        it 'should set alias_ids for enrollee' do
          transformed_target_xml.xpath("//cv:enrollment_event_body/cv:affected_members/cv:affected_member", xml_namespace).each do |node|
            node.xpath("cv:member/cv:id/cv:alias_ids/cv:alias_id/cv:id", xml_namespace).each do |id_node|
              expect(alias_ids).to include(id_node.content)
            end
      ...
      context 'enrollee with carrier_member_id and without carrier_policy_id' do
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, :c_id => '123456', cp_id: nil) }

        it 'should set carrier_member_id alias_ids for enrollee' do
          nodes = transformed_target_xml.xpath("//cv:enrollment_event_body/cv:affected_members/cv:affected_member", xml_namespace)
          nodes.each do |node|
            id_node = node.xpath("cv:member/cv:id/cv:alias_ids/cv:alias_id/cv:id", xml_namespace).first
            if node.xpath("cv:member/cv:id/cv:id", xml_namespace).first.content.split('#').last == '1'
              # Case for member 1
              expect(id_node.content).to eq(carrier_member_id)
            else
              # Case for member 2
              expect(id_node).to be_nil
            end
          end
        end
      end

      context 'enrollee without carrier_member_id and with carrier_policy_id' do
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, c_id: nil, cp_id: '7123456') }

        it 'should set carrier_policy_id alias_ids for enrollee' do
          nodes = transformed_target_xml.xpath("//cv:enrollment_event_body/cv:affected_members/cv:affected_member", xml_namespace)
          nodes.each do |node|
            id_node = node.xpath("cv:member/cv:id/cv:alias_ids/cv:alias_id/cv:id", xml_namespace).first
            if node.xpath("cv:member/cv:id/cv:id", xml_namespace).first.content.split('#').last == '1'
              # Case for member 1
              expect(id_node.content).to eq(carrier_policy_id)
            else
              # Case for member 2
              expect(id_node).to be_nil
            end
      ...
      context 'enrollee without carrier_member_id and without carrier_policy_id' do
        let!(:enrollee_primary) { double(:m_id => 1, :coverage_start => :one_month_ago, c_id: nil, cp_id: nil) }

        it 'should not set any alias_ids for enrollee' do
          transformed_target_xml.xpath("//cv:enrollment_event_body/cv:affected_members/cv:affected_member", xml_namespace).each do |node|
            expect(node.xpath("cv:member/cv:id/cv:alias_ids", xml_namespace)).to be_empty
          end
        end
      end
    end
    ...
    context 'set carrier assigned ids at policy level' do
      ...
      context 'enrollee with carrier_member_id and without carrier_policy_id' do
        ...
        it 'should set carrier_member_id alias_ids for enrollee' do
          nodes = transformed_target_xml.xpath("//cv:enrollment_event_body/cv:enrollment/cv:policy/cv:enrollees/cv:enrollee", xml_namespace)
          nodes.each do |node|
            id_node = node.xpath("cv:member/cv:id/cv:alias_ids/cv:alias_id/cv:id", xml_namespace).first
            if node.xpath("cv:member/cv:id/cv:id", xml_namespace).first.content.split('#').last == '1'
              # Case for member 1
