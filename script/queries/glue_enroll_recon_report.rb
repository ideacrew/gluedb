...
if File.exists?(ARGV[0]) && "#{Rails.root}/#{ARGV[0]}" == file_name
  ...
  CSV.open("glue_enrollment_recon_report_#{timestamp}.csv", 'w') do |csv|
    csv << ["Primary Member ID", "Member ID", "Policy ID", "Policy Subscriber ID ", "Status", "Member Status",
              ...
              "Purchase Date", "Coverage Start", "Coverage End", "Home Address", "Mailing Address","Work Email", 
              "Home Email", "Phone Number","Broker", "Broker NPN", "Race", "Ethnicity", "Citizen Status",
              "Broker Assisted", "In Glue", "Glue eg_id", "Glue policy status", "Glue NPT status","Glue Total Premium", 
              "Glue APTC value", "Glue Responsible Amt ", "Glue Rating area", "Glue Policy start", "Glue Policy End", "Glue Broker"]
    ...
    CSV.foreach(file_name, headers: true).each do |row|
      ...
      begin
        ...
        if pol.present?
          ...
          in_glue = pol.present?
          csv << [row["Primary Member ID"], row["Member ID"], row["Policy ID"], row["Policy Subscriber ID "],
                  row["Status"], row["Member Status"], row["First Name"], row["Last Name"], row["SSN"],
                  row["DOB"], row["Age"], row["Gender"], row["Relationship"], row["Benefit Type"],
                  ...
                  row["Race"], row["Ethnicity"], row["Citizen Status"], row["Broker Assisted"], 
                  in_glue, pol.eg_id, pol.aasm_state, pol.term_for_np, pol.pre_amt_tot, pol.applied_aptc, 
                  pol.tot_res_amt, pol.rating_area, pol.policy_start, pol.policy_end, broker]
        else
          csv << [row["Primary Member ID"], row["Member ID"], row["Policy ID"], row["Policy Subscriber ID "],
                  row["Status"], row["Member Status"], row["First Name"], row["Last Name"], row["SSN"],
                  row["DOB"], row["Age"], row["Gender"], row["Relationship"], row["Benefit Type"],
